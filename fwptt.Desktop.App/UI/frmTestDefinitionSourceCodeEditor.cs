...
namespace fwptt.Desktop.App.UI
{
    ...
    public partial class frmTestDefinitionSourceCodeEditor : Form, UI.IItemEditor<TestDefinition>
	{
        ...
        private void btnSaveSourceCode_Click(object sender, EventArgs e)
        {
            try
            {
                TestProjectHost.Current.SaveTestProjectDefinitionCSharpCode(CurrentItem, txtSourceCode.Text);
            }
            catch (Exception ex)
            {
        ...
        void BtnCompileCodeClick(object sender, EventArgs e)
        {
            try
            {
                TestProjectHost.Current.CreateMemoryAssembly(txtSourceCode.Text, CurrentItem.Assemblies);
